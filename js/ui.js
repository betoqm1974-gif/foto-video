(function(){
 ...
 // Desativar botÃÂ£o direito do rato em todas as pÃÂ¡ginas
 // (nota: nÃÂ£o ÃÂ© uma proteÃÂ§ÃÂ£o absoluta, mas evita o menu contextual comum)
 ...
 function init(){
  ...
  (function initCursorGlow(){
    try{
      ...
      // no fim do body para ficar por cima do header/rodapé (sem bloquear cliques)
      ...
      if(isFinePointer){
        ...
        document.addEventListener('mouseleave', () => setXY(-9999, -9999));
      }
      ...
      if(isCoarsePointer){
        ...
        window.addEventListener('touchend', () => setXY(-9999, -9999), { passive: true });
        window.addEventListener('touchcancel', () => setXY(-9999, -9999), { passive: true });
      }
    }catch(e){}
  })();
...
(function(){
  ...
  const buildCandidates = (src) => {
    ...
    // Nomes: original, sem _ inicial, e variaÃÂ§ÃÂµes de caixa (ÃÂºtil em servers case-sensitive)
    const names = [];
    const pushName = (nm) => {
      if(!nm) return;
  ...
  const buildWatermarkedDataURL = async (photoSrc) => {
    ...
    // Nota: usedSrc pode ter sido ajustado (extensÃÂ£o/maiÃÂºsculas) para compatibilidade em servidores case-sensitive.
    ...
    // Marca de ÃÂ¡gua ao centro, mais pequena e semi-transparente
    ...
    ctx.globalAlpha = 0.35; // menos transparente do que o fix_20, mais do que o inÃÂ­cio
  ...
  const buildWatermarkedThumbDataURL = async (photoSrc) => {
    // Reutiliza a mesma marca de ÃÂ¡gua usada na popup
    const lb = document.getElementById('lightbox');
    const wmEl = lb ? lb.querySelector('.lightbox__wm') : null;
  ...
  // Expor funÃÂ§ÃÂ£o para a init() aplicar marca de ÃÂ¡gua ÃÂ s miniaturas da Galeria
...
const openLb = async (src, alt, shouldWatermark, compareSrc, compareStart, galleryCtx) => {
  ...
  // se uma miniatura falhar, tenta variaÃÂ§ÃÂµes comuns (.JPG/.JPEG e remoÃÂ§ÃÂ£o de _ inicial).
